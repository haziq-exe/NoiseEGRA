...
#
#   scripts/modal_next_runs.sh wholefive <name>
#   scripts/modal_next_runs.sh wholeloop <name>
#
...
SUITE="${1:?usage: modal_next_runs.sh <wholefive|wholeloop> <name>}"
NAME="${2:?usage: modal_next_runs.sh <wholefive|wholeloop> <name>}"
...
modal run scripts/modal_run.py --name "$NAME" --shards 2 --runner-args "$ARGS"
